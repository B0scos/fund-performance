    ...
    for model in models:
        for cluster in clusters:
            for type_process in pre_processing_flags:
                ...
                train_results = evaluate(df_train, look_features=look_features)
                train_results['dataset'] = 'train'
                print("\nTrain Results:")
                print(train_results)

                test_results = evaluate(df_test, look_features=look_features)
                test_results['dataset'] = 'test'
                print("\nTest Results:")
                print(test_results)

                # Add experiment parameters to the results
                for results_df in [train_results, test_results]:
